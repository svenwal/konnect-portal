<template>
    <div :ref="specDetails" class="spec mt-6 api-documentation">
        ...
        <div class="container mx-auto max-w-screen-2xl px-5 md:px-0">
            <EmptyState v-if="hasProductError" is-error class="mt-6" :message="hasProductError" />
        </div>
        ...
        <TicketForm :visible="raiseTicket" :paths="specPaths" @submitted="handleSubmitted" @cancelled="handleCancelled">
        </TicketForm>
        <!-- JS -->

        <div v-if="hasProductError" class="spec-render-error" />
        ...
        <SpecDetails v-else-if="spec" class="w-100" :document="spec" :has-sidebar="false"
            :application-registration-enabled="applicationRegistrationEnabled"
            :active-operation="sidebarActiveOperationListItem" :current-version="currentVersion?.name"
            @clicked-view-spec="triggerViewSpecModal" @clicked-register="triggerViewSpecRegistrationModal" />

        <ViewSpecModal :is-visible="viewSpecModalIsVisible" :spec-contents="specContents" :spec-name="specName"
            :download-callback="downloadSpecContents" @close="closeModal" />
        <ViewSpecRegistrationModal :initial-selected-application="($route.query.application as string)"
            :is-visible="viewSpecRegistrationModalIsVisible" :product="product" :version="currentVersion"
            @close="closeModal" />
    </div>
</template>
