<template>
  <KModal
        ...
        <template v-slot:header-content>
          ...
          {{ ticketLanguage.raiseButtonText }}
        </template>
        <template v-slot:body-content>
          <div>
            {{ ticketLanguage.formSubtext}}
          </div>
          <div style="height: 10px;"></div>
          <KInput class="w-100" :label="ticketLanguage.formEmail" :placeholder="ticketLanguage.formEmail"  v-model="formData.developerId" />
          <div style="height: 10px;"></div>
          <KSelect class="w-100" :label="ticketLanguage.formPriority" :items="ticketPriority" v-model="formData.priority" />
          <div style="height: 10px;"></div>
          <KSelect class="w-100" :label="ticketLanguage.formServicePath" :items="paths" v-model="formData.servicePath"/>
          <div style="height: 10px;"></div>
          <KSelect class="w-100" :label="ticketLanguage.formRequestType" :items="ticketItems" v-model="formData.requestType"/>
          <div style="height: 10px;"></div>
          <KTextArea class="w-100" :label="ticketLanguage.formDetails" v-model="formData.issueDetails"/>
        </template>
        <template v-slot:action-buttons>
          <KButton
            appearance="primary"
            @click="handleSubmit(formData)">{{ ticketLanguage.formRaise }}</KButton>
        </template>
    </Kmodal>
...
data () {
  ...
  return {
    ticketItems, ticketPriority, email, ticketLanguage
  }
}
}
